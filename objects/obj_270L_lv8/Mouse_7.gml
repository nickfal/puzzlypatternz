...
location[0] = 0;
location[1] = 0;
least_dist = sqrt(sqr(x - obj_goal8.x) + sqr(y - obj_goal8.y));

// Finds the closest value in obj_goal3.goal_design to this object's
//       x and y
for(row = 0; row < array_length(obj_goal8.goal_design); ++row){
	for(col = 0; col < array_length(obj_goal8.goal_design[0]); ++col){
		x_dist = x - (obj_goal8.x + col*_dist);
		y_dist = y - (obj_goal8.y + row*_dist);
...
if(obj_goal8.goal_design[location[0]][location[1]] == 1) {
	still_valid = true;
	...
	for(row = 0; row < array_length(design); ++row){
		for(col = 0; col < array_length(design[0]); ++col){
			if(design[row][col] == 1) {
				...
				// Outside of goal:
				else if(obj_goal8.goal_design[location[0] +
					row - 1][location[1] + col - 1] == 0) still_valid = false;
				
				// Spot is already filled by other block
				else if(obj_goal8.goal_filled[location[0] +
	...
	if(still_valid) {
		x = obj_goal8.x + location[1]*_dist;
		y = obj_goal8.y + location[0]*_dist;
		for(row = 0; row < array_length(design); ++row){
			for(col = 0; col < array_length(design[0]); ++col){
				if(design[row][col] == 1) {
					obj_goal8.goal_filled[location[0] +
