/// @description Snaps piece to closest goal block of obj_goal1
...
if(obj_goal1.goal_design[location[0]][location[1]] == 1) {
	...
	for(row = 0; row < array_length(design); ++row){
		for(col = 0; col < array_length(design[0]); ++col){
			if(design[row][col] == 1) {
				...
				// Outside of goal:
				else if(obj_goal1.goal_design[location[0] +
					row - 1][location[1] + col - 1] == 0) still_valid = false;
				
				// Spot is already filled by other block
				else if(obj_goal1.goal_filled[row][col] == 1) still_valid = false;
			}
		}
	}
	...
	if(still_valid) {
		...
		for(row = 0; row < array_length(design); ++row){
			for(col = 0; col < array_length(design[0]); ++col){
				if(design[row][col] == 1) {
					obj_goal1.goal_filled[location[0] +
