...
if (on_piece) {
	...
	if(in_place) {
		for(row = 0; row < array_length(design); ++row){
			for(col = 0; col < array_length(design[0]); ++col){
				var _dist = 44;
				goal_row = round((y - obj_goal7.y) / _dist);
				goal_col = round((x - obj_goal7.x) / _dist);
				if(design[row][col] == 1) obj_goal7.goal_filled[goal_row +
	...
	if(!mouse_grabbed) {
		x = mouse_x;
		y = mouse_y;
		effect_create_above(ef_ellipse, mouse_x,mouse_y,50,c_black);
		mouse_grabbed = true;
		grab = true;
		depth = -2;
